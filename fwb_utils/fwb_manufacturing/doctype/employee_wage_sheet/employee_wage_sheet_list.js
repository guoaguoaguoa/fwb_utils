...
frappe.listview_settings["Employee Wage Sheet"] = {
	onload(listview) {
		if (listview._fwb_wage_date_filter_setup) {
			return;
		}
		listview._fwb_wage_date_filter_setup = true;

		const from_date_field = listview.page.add_field({
			label: "工资开始",
			fieldtype: "Date",
			fieldname: "fwb_wage_from_date_filter"
		});

		const to_date_field = listview.page.add_field({
			label: "工资结束",
			fieldtype: "Date",
			fieldname: "fwb_wage_to_date_filter"
		});
		...
		const workstation_field = listview.page.add_field({
			...
			fieldtype: "Link",
			options: "Workstation",
			fieldname: "fwb_wage_workstation_filter"
		});
		...
		const controlled_filters = [
			["Employee Wage Sheet", "from_date"],
			["Employee Wage Sheet", "to_date"],
		...
		const apply_quick_filters = () => {
			...
			const apply = () => {
				...
				const filters = [];
				if (from_date) {
					filters.push(["Employee Wage Sheet", "from_date", ">=", from_date]);
				}
				if (to_date) {
					filters.push(["Employee Wage Sheet", "to_date", "<=", to_date]);
				}
				if (workstation) {
